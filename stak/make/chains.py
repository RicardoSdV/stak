...
def makeDataEntry(frame, keyValPairsForLogging, kwargsForLogging):
    ...
    return (dataEntryFlag, timeClock, len(data)) + data


def makeCallChainEntry(frame, entryData=None):
    if entryData:
        callChainEntry = [dataChainEntryFlag, timeClock(), len(entryData)]
        callChainEntry.extend(entryData)
    else:
        callChainEntry = [callChainEntryFlag, timeClock()]
