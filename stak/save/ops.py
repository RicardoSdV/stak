from ..state import stakLog, config
...
def joinSplitLinksAndLeaveOutSilencedFiles():  # type: () -> Lst[Str4, Tup[str, ...]]
    ...
    for stamp, theRest in stakLog:
        ...
        if firstFrameFilePath:
            if firstFrameFilePath in config['silentFiles']:
                continue

            if config['loudFiles']:
                if not any(path in firstFrameFilePath for path in config['loudFiles']):
                    addSilencedFound(firstFrameFilePath)
                    continue

            if config['silentFiles']:
                if any(path in firstFrameFilePath for path in config['silentFiles']):
                    addSilencedFound(firstFrameFilePath)
                    continue
