...
from         .z_utils                import write, writePickle, read, readPickle, timeCall, log
...
def joinSplitLinksAndLeaveOutSilencedFiles(
    ...
    repCnts = countFirstFrameLines(so.repeatSilence, False)
    repFrames = {frame for frame, cnt in repCnts}
    ...
    for stamp, theRest in stakLog:
        firstFrame = theRest[0]
        if theRest.__class__ is not str and firstFrame in repFrames:
            continue

        firstFrameFilePath = firstFrame[0]
...
def countFirstFrameLines(n=10, _print=True):
    counter = defaultdict(int)

    for stamp, theRest in stakLog:
        if theRest.__class__ is str:
            continue

        firstFrame = theRest[0]
        counter[firstFrame] += 1

    counter = counter.items()
    counter.sort(key=lambda t: t[1], reverse=True)
    counter = counter[:n]

    if _print:
        print '[STAK] Counting most repeated first frames'
        for cnt in counter:
            print cnt
    else:
        return counter
